...
read_centroid = pd.read_csv("centroids.csv")
...
def aggiornaFeatures():
  ...
  if totalPkt != 0:   
    if distance_Y > threshold:
        ...
        logging.warning(" Abnormal traffic detected - Date: {}, Time: {}".format(datetime.datetime.now().date(), datetime.datetime.now().time()))
  ...
  if len(classPrediction) == len(lClass):
    df12 = pd.DataFrame({"Predicted class ":classPrediction,"Right classe ":lClass})
